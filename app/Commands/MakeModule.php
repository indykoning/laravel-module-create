...
class MakeModule extends Command
{
    ...
    {
        $getPath = (new Process(['pwd']));
        $getPath->run();
        $path = trim(preg_replace('/\s\s+/', ' ', $getPath->getOutput()));
        if(!File::exists($path.'/artisan')) {
            $this->warn('This tool can only be used for Laravel projects.');
            return 0;
        }
        ...
        $relPath = $this->relPath = __(':path/Modules/:vendor/:package', @compact('path', 'vendor', 'package'));
        $packagePath = $this->packagePath = $this->relPath;
        ...
        $packagePath = trim(getcwd()) . __('/Modules/:vendor/:package', @compact('vendor', 'package'));

        Process::fromShellCommandline(__('composer config repositories.:jsonVendor-:jsonPackage path ":packagePath" --file composer.json', @compact('packagePath', 'jsonVendor', 'jsonPackage')))
        ->setTty(true)
        ->setTimeout(null)
        ->run();
        $this->newLine();
